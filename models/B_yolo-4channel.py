...
class Model(nn.Module):
    def __init__(self, cfg='yolov5n-4channel.yaml', ch=4, nc=None):  # model, input channels, number of classes
...
if __name__ == '__main__':
    ...
    model = Model(opt.cfg, ch= 4).to(device)
    #model = Model(opt.cfg).to(device)
    model.eval()
    ...
    #input_tensor = torch.randn(1, 1, 640, 640).to(device)
    input_tensor = torch.randn(1, 4, 640, 640).to(device)
    ...
    #input_tensor = torch.randn(batch_size, 1, 640, 640).to(device)
    input_tensor = torch.randn(batch_size, 4, 640, 640).to(device)
